...
class LH26VideoUpscaler(io.ComfyNode):
    """
    LH2.6 Video Upscaler ComfyUI Node
    ...
    def define_schema(cls) -> io.Schema:        
        return io.Schema(
            node_id="LH26VideoUpscaler",
            display_name=f"LH2.6 Video Upscaler (v{__version__})",
            category="LH2.6",
            description=(
                "LH2.6 main upscaling node: processes video frames using DiT and VAE models with diffusion-based enhancement. "
            ...
            inputs=[
                ...
                io.Custom("LH26_DIT").Input("dit",
                    tooltip="DiT model configuration from LH2.6 (Down)Load DiT Model node"
                ),
                io.Custom("LH26_VAE").Input("vae",
                    tooltip="VAE model configuration from LH2.6 (Down)Load VAE Model node"
                ),
                io.Int.Input("seed",
                    default=42,
                    min=0,
    ...
    def execute(cls, image: torch.Tensor, dit: Dict[str, Any], vae: Dict[str, Any], 
        ...
        """
        Execute LH2.6 video upscaling with progress reporting
        
        Main entry point for ComfyUI node execution.
        Automatically detects and preserves input format (RGB or RGBA). Validates model files,
        configuration unpacking, and delegates to upscaling pipeline.
        
        Args:
            image: Input video frames as tensor (N, H, W, C) in [0, 1] range
            dit: DiT model configuration from LH26LoadDiTModel node
            vae: VAE model configuration from LH26LoadVAEModel node
