"""
Downloads utility module for LH2.6
...
def download_weight(dit_model: str, vae_model: str, model_dir: Optional[str] = None, debug=None) -> bool:
    """Validate the separately installed LH2.6 DiT and VAE model files."""
    ...
    for filename, model_info in files_to_download:
        ...
        # LH2.6 weights are supplied through RunningHub's model review/upload
        # flow rather than embedded in or downloaded by the custom node.
        if not repo:
            if debug:
                debug.log(
        ...
        # Download file (kept for explicitly registered mirrors)
